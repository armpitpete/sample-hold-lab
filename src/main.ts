...
const lessons:Lesson[]=[
  ...
  {title:'Create related companions',instruction:'Companion Hold creates high and low outputs around the sampled centre.',source:'noise',mode:'companion-hold',rate:1,slew:.35,jitter:.08,spread:1.2}
];
...
<section class="lab-grid">
  ...
  <main class="workbench">
    ...
    <section class="hear"><div><p class="eyebrow">Hear the same control signal</p><h2>Safe pitch mapping</h2><p>Only the slewed main output controls one quiet sine oscillator.</p></div><div><output id="audioStatus">Audio stopped</output><output id="pitch">220 Hz</output><button id="audioStart">Start audio</button><button id="audioStop">Panic / stop</button></div></section>
  </main>
</section>`;
...
const canvas=$<HTMLCanvasElement>('#scope'),ctx=canvas.getContext('2d');if(!ctx)throw new Error('Canvas unavailable');
...
let mode:HoldMode='sample-hold',inputSource:Source='lfo',raw=0,smoothed=0,gate=false,nextEvent=0,last=0,lessonIndex=0,noise=0,audio:AudioContext|null=null,osc:OscillatorNode|null=null,gain:GainNode|null=null;
const history:Point[]=[];
...
function explain(kind:string,input:number){const title=$<HTMLElement>('#eventTitle'),text=$<HTMLElement>('#eventText');if(mode==='track-hold'){title.textContent=gate?'Gate opened: tracking':'Gate closed: holding';text.textContent=gate?`The output is now following the input. Current value: ${input.toFixed(2)} V.`:`The gate closed, so ${raw.toFixed(2)} V remains held.`}else{title.textContent=kind==='manual'?'Manual trigger captured the input':'Clock trigger captured the input';text.textContent=`The trigger captured ${raw.toFixed(2)} V. It remains the target until the next trigger${Number(slewInput.value)>0?', while slew moves the output toward it.':'.'}`}}
